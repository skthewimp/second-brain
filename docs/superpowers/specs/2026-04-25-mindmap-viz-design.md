...
4. **`noteCount`** is computed deterministically by Swift, **not** by the LLM. Source of truth is the `source_count` field in each theme page's YAML frontmatter (already maintained by `VaultWriter.bumpFrontmatter` on every ingest). The engine builds `noteCounts: [themeSlug: Int]` from `wiki/themes/*.md` frontmatter and passes it into the prompt as read-only context. **Scope:** only top-level theme nodes (whose `id` matches a theme slug exactly) get a deterministic count. Sub-theme nodes deeper than level 1 carry `noteCount = 0` in v1 — no deterministic source exists below the theme level, and we explicitly do not let the LLM invent counts. Rationale: cheaper, deterministic, eliminates a class of LLM arithmetic errors. The `NodeOp.update` payload therefore omits `noteCount` — the LLM cannot mutate it.
